...
def naive_merge(sections, chunk_token_num=128, delimiter="\n。；！？"):
    ...
    def add_chunk(t, pos):
        ...
        if t.find(pos) < 0:
            t += pos
        # Start a new chunk if appending the next section would overflow.
        if not cks or tk_nums[-1] + tnum > chunk_token_num:
            cks.append(t)
            tk_nums.append(tnum)
            return
        cks[-1] += t
...
def naive_merge_with_images(texts, images, chunk_token_num=128, delimiter="\n。；！？"):
    ...
    def add_chunk(t, image, pos=""):
        ...
        if t.find(pos) < 0:
            t += pos
        # Keep figure sections isolated to avoid merging nearby footnotes or
        # following paragraphs into the same image chunk.
        if image is not None:
            cks.append(t)
            ...
            return
        # Start a new chunk if appending the next section would overflow.
        if not cks or result_images[-1] is not None or tk_nums[-1] + tnum > chunk_token_num:
            cks.append(t)
            result_images.append(image)
            tk_nums.append(tnum)
            return
