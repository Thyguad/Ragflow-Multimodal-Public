...
from rag.image_index import build_doc_image_index_safe
...
def _build_doc_image_sidecar_background(task):
    try:
        kb_parser_config = task.get("kb_parser_config") or {}
        if not is_multimodal_image_retrieval_enabled(kb_parser_config):
            return
        doc_name = normalize_doc_base_name(task.get("name"))
        if not doc_name:
            return
        build_doc_image_index_safe(doc_name, force_rebuild=False)
    except Exception:
        logging.exception("Failed to kick off multimodal image sidecar build for document %s", task.get("name"))


def _start_doc_image_sidecar_task(task):
    if task.get("task_type"):
        return
    worker = threading.Thread(
        target=_build_doc_image_sidecar_background,
        args=(copy.deepcopy(task),),
        name=f"image_sidecar_{task.get('doc_id', 'unknown')}",
        daemon=True,
    )
    worker.start()
...
async def do_handle_task(task):
    ...
    _start_doc_image_sidecar_task(task)
    logging.info(
