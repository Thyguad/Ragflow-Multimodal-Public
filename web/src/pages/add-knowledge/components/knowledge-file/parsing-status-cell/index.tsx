...
const PopoverContent = ({ record }: IProps) => {
  ...
  const imageIndexStatus = record.image_index_status?.status || 'pending';

  const imageIndexStatusConfigMap: Record<
    string,
    { color: string; label: string }
  > = {
    disabled: {
      color: 'default',
      label: t('imageIndexStatusDisabled'),
    },
    waiting_for_parse: {
      color: 'cyan',
      label: t('imageIndexStatusWaitingForParse'),
    },
    pending: {
      color: 'processing',
      label: t('imageIndexStatusPending'),
    },
    ready: {
      color: 'success',
      label: t('imageIndexStatusReady'),
    },
    failed: {
      color: 'error',
      label: t('imageIndexStatusFailed'),
    },
    no_figures: {
      color: 'default',
      label: t('imageIndexStatusNoFigures'),
    },
    no_indexable_figures: {
      color: 'default',
      label: t('imageIndexStatusNoIndexableFigures'),
    },
  };
  const imageIndexStatusConfig =
    imageIndexStatusConfigMap[imageIndexStatus] ||
    imageIndexStatusConfigMap.pending;
  ...
  const items: DescriptionsProps['items'] = [
    ...
    {
      key: 'image_index_status',
      label: t('imageIndexStatus'),
      children: (
        <Flex vertical gap={6}>
          <Tag color={imageIndexStatusConfig.color}>
            {imageIndexStatusConfig.label}
          </Tag>
          {record.image_index_status?.error ? (
            <div className={styles.popoverContentText}>
              {record.image_index_status.error}
            </div>
          ) : null}
        </Flex>
