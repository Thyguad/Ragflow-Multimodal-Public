...
export const useSubmitCustomModel = () => {
  const [selectedLlmFactory, setSelectedLlmFactory] = useState<string>('');
  const { addLlm, loading } = useAddLlm();
  const {
    visible: customModelAddingVisible,
    hideModal: hideCustomModelAddingModal,
    showModal: showCustomModelAddingModal,
  } = useSetModalState();

  const onCustomModelAddingOk = useCallback(
    async (payload: IAddLlmRequestBody) => {
      const ret = await addLlm(payload);
      if (ret === 0) {
    ...
    [hideCustomModelAddingModal, addLlm],
  );

  const handleShowCustomModelAddingModal = (llmFactory: string) => {
    setSelectedLlmFactory(llmFactory);
    showCustomModelAddingModal();
  };

  return {
    customModelAddingLoading: loading,
    onCustomModelAddingOk,
    customModelAddingVisible,
    hideCustomModelAddingModal,
    showCustomModelAddingModal: handleShowCustomModelAddingModal,
    selectedCustomModelLlmFactory: selectedLlmFactory,
  };
};
