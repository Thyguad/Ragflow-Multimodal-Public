...
def add_llm():
    ...
    if not api_key:
        saved_llms = TenantLLMService.query(tenant_id=current_user.id, llm_factory=factory)
        saved_llm = next((llm for llm in saved_llms if llm.api_key), None)
        if saved_llm:
            api_key = saved_llm.api_key
            if not req.get("api_base"):
                req["api_base"] = saved_llm.api_base or ""
        else:
            api_key = "x"
