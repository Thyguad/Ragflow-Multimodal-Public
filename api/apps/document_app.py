...
from rag.image_index import get_doc_image_index_status
...
def list_docs():
    ...
    try:
        ...
        image_retrieval_enabled = is_multimodal_image_retrieval_enabled(getattr(kb, "parser_config", None) if kb else None)

        for doc_item in docs:
            if doc_item["thumbnail"] and not doc_item["thumbnail"].startswith(IMG_BASE64_PREFIX):
                doc_item["thumbnail"] = f"/v1/document/image/{kb_id}-{doc_item['thumbnail']}"
            if image_retrieval_enabled:
                image_index_status = get_doc_image_index_status(normalize_doc_base_name(doc_item.get("name")))
                if image_index_status.get("status") == "pending" and doc_item.get("run") == TaskStatus.RUNNING.value:
                    image_index_status["status"] = "waiting_for_parse"
            else:
                image_index_status = {"status": "disabled"}
            doc_item["image_index_status"] = image_index_status
