...
def doc_upload_and_parse(conversation_id, file_objs, user_id):
    ...
    def build_image_sidecar_async(doc_name: str):
        if not is_multimodal_image_retrieval_enabled(kb.parser_config):
            return
        normalized_doc = normalize_doc_base_name(doc_name)
        if not normalized_doc:
            return
        threading.Thread(
            target=build_doc_image_index_safe,
            args=(normalized_doc,),
            kwargs={"force_rebuild": False},
            name=f"upload_image_sidecar_{normalized_doc}",
            daemon=True,
        ).start()
    ...
    for doc_id in docids:
        ...
        build_image_sidecar_async(doc_nm[doc_id])
