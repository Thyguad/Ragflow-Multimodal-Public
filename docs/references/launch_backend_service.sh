...
# Function to start the image_server
run_image_server() {
    local retry_count=0
    # local BASE_PYTHON="/opt/miniconda3/bin/python"
    while ! $STOP && [ $retry_count -lt $MAX_RETRIES ]; do
        echo "Starting image_server.py (Attempt $((retry_count+1)))"
        # $BASE_PYTHON image_server.py
        $PY image_server.py
        EXITCODE=$?
        if [ $EXITCODE -eq 0 ]; then
            echo "image_server.py exited successfully."
            break
        else
            echo "image_server.py failed with exit code $EXITCODE. Retrying..." >&2
            retry_count=$((retry_count + 1))
            sleep 2
        fi
    done
    ...
    if [ $retry_count -ge MAX_RETRIES ]; then
        echo "image_server.py failed after $MAX_RETRIES attempts. Exiting..." >&2
        cleanup
    fi
}
